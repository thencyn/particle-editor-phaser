...
import { IConfiguracionAdicional, IConfiguracionEmitter, IDestroyable, IGuardarConfiguracionEmitter, IGuardarConfiguracionProyectos, IMemoria, IProyectosBasicosConfiguracion } from "../Utilidades/Interfaces";
...
export default class PrincipalScene extends Phaser.Scene {
	...
	public create(): void {
		...
		this.registry.events.on(Eventos.Exportar, (tipo: 'proyecto' | 'proyectojs' | 'emitter' | 'emitters') => {
			if (tipo === 'proyecto') {
				...
				const listaEmittersExportar = this.listaEmitters.map((item, index) => {
					...
					return {
						...
						memoria: memoria,
						listaDeathZones: item.configAdicional.listaDeathZones,
						listaEmitZones: item.configAdicional.listaEmitZones,
						emitZoneAbsolutoX: item.configAdicional.emitZoneAbsolutoX,
						emitZoneAbsolutoY: item.configAdicional.emitZoneAbsolutoY
					};
				});
				...
				this.eliminarPropiedadesEmitter(configParticulas, configAdicional)
				configParticulas.frame = !this.listaEmitters[this.indexEmitterSeleccionado].frameCycle ? this.listaEmitters[this.indexEmitterSeleccionado].frame : { frames: this.listaEmitters[this.indexEmitterSeleccionado].frame, cycle: true };
				const emitter = {
					x: this.listaEmitters[this.indexEmitterSeleccionado].emitter.x,
					y: this.listaEmitters[this.indexEmitterSeleccionado].emitter.y,
					texture: this.listaEmitters[this.indexEmitterSeleccionado].texture,
					frame: this.listaEmitters[this.indexEmitterSeleccionado].frame,
					configParticle: configParticulas,
					memoria: memoria,
					listaDeathZones: this.listaEmitters[this.indexEmitterSeleccionado].configAdicional.listaDeathZones,
					listaEmitZones: this.listaEmitters[this.indexEmitterSeleccionado].configAdicional.listaEmitZones,
					emitZoneAbsolutoX: this.listaEmitters[this.indexEmitterSeleccionado].configAdicional.emitZoneAbsolutoX,
					emitZoneAbsolutoY: this.listaEmitters[this.indexEmitterSeleccionado].configAdicional.emitZoneAbsolutoY,
				};
		...
		this.registry.events.on(Eventos.AbrirNuevoEjemplo, (listaEmitters: IGuardarConfiguracionEmitter[]) => {
			this.abrirProyectoEjemplo(listaEmitters);
		...
		this.registry.events.on(Eventos.Importar, (proyecto: IGuardarConfiguracionProyectos) => {
			...
			this.abrirProyectoEjemplo(proyecto.listaEmitters);
		});
	...
	private crearPanel() {
		...
		this.panel.on('valuechange', (newValue, oldValue, bindingTarget, bindingKey: string) => {
			...
			this.resetEmitter(this.indexEmitterSeleccionado, ['tipoSpeedSeleccionada'].includes(bindingKey) || resetColor || resetBounds || resetGravityWell || resetPropiedadBasica || resetEmitZoneCambioXY || resetStopAfterDuration || configParticle.hasOwnProperty('bounce'));
		});
	}
	...
	private crearNuevoEmitter() {
		const configParticle: Phaser.Types.GameObjects.Particles.ParticleEmitterConfig = {
			...
			sortOrderAsc: false,
			// emitZone: [
			// 	// { type: 'edge', source: new Phaser.Geom.Line(0, 0, 0, 160), quantity: 64, total: 1 },
			// 	// { type: 'edge', source: new Phaser.Geom.Line(0, 0, 160, 0), quantity: 64, total: 1 },
			// 	// { type: 'edge', source: new Phaser.Geom.Rectangle(0, 0, 160, 160), quantity: 64, total: 1 },
			// 	// { type: 'random', source: new Phaser.Geom.Circle(0, 0, 25) }
			// 	{ type: 'random', source: new Phaser.Geom.Rectangle(-100, 0, 100, 100), total: 0 },
			// 	{ type: 'random', source: new Phaser.Geom.Rectangle(100, 0, 100, 100), total: -1 }
			// ]
		};
	...
	private cambiarTipoVelocidad() {
		...
		if (configAdicional.tipoSpeedSeleccionada === 'SpeedAngle') {
			this.cambiarPropiedadesSpeedEmitter(configAdicional.speedSeleccionada === 'Unica' ? 'speed_unica' : configAdicional.speedSeleccionada === 'Transition' ? 'speed_start' : 'speed_min');
			this.cambiarPropiedadesCompuestasEmitter(`angle_${configAdicional.angleSeleccionada === 'Random' ? 'min' : 'start' }`);
		} else if (configAdicional.tipoSpeedSeleccionada === 'SpeedXY') {
			this.cambiarPropiedadesSpeedEmitter(configAdicional.speedXSeleccionada === 'Unica' ? 'speedX_unica' : configAdicional.speedXSeleccionada === 'Transition' ? 'speedX_start' : 'speedX_min');
			this.cambiarPropiedadesSpeedEmitter(configAdicional.speedYSeleccionada === 'Unica' ? 'speedY_unica' : configAdicional.speedYSeleccionada === 'Transition' ? 'speedY_start' : 'speedY_min');
		}
	}
	...
	private cambiarPropiedadesPosicionEmitter(key: string) {
		...
		if (key.endsWith('_min') || key.endsWith('_max')) {
			...
			//FIXME: VALDIAR INGRESO CORRECTO
			// configParticle[propiedadParticula] = memoria[propiedad][`${propiedad}_puntos`];
	...
	private cambiarPropiedadesCompuestasEmitter(keyPropiedad: string) {
		...
		const tipo: 'Unica' | 'Transition' | 'Random' = accionTipoSeleccionado[keyPropiedad.split('_')[1]];
	...
	private crearPropiedadesPanel(configAdicional: IConfiguracionAdicional, memoria: IMemoria) {
		...
		return [{
				...
				{
					$type: 'folder', title: '✔️Scale',
					expanded: memoria.expandirPaginasUtilizadas,
					$properties: [
						{
							$type: 'tab',
							pages: [
								{
									...
									$properties: [
										{ $key: 'scale.scale_unica', title: 'Scale ', min: 0, max: 2, step: 0.01 },
									],
								},

								{
									...
									$properties: [
										{ $key: 'scale.scale_start', title: 'Start ', min: 0, max: 2, step: 0.01 },
										{ $key: 'scale.scale_end', title: 'End ', min: 0, max: 2, step: 0.01 },
								...
								},
								{
									...
									$properties: [
										{ $key: 'scale.scale_min', title: 'Min ', min: 0, max: 2, step: 0.01 },
										{ $key: 'scale.scale_max', title: 'Max ', min: 0, max: 2, step: 0.01 },
									]
								}
							],
						},
				...
				{
					$type: 'folder', title: `${memoria.bokeh.bokeh_activado ? '✔️' : ''}Bokeh`,
	...
	private proyectoAbrir(id: number) {
	...
	private abrirProyectoEjemplo(listaEmittersAbrir: IGuardarConfiguracionEmitter[]) {
		// const proyecto = UtilProyectos.leerProyecto(id);
	...
	private obtenerConfigParticleDesdeMemoria(configAdicional: IConfiguracionAdicional, memoria: IMemoria): Phaser.Types.GameObjects.Particles.ParticleEmitterConfig {
		...
		const obtenerPropiedadesCompuestas = (propiedad: string, valor: 'Unica' | 'Transition' | 'Random') => {
			if (valor === 'Unica') {
				return memoria[propiedad][`${propiedad}_unica`];
			} else if (valor === 'Transition') {
				const aux = { start: memoria[propiedad][`${propiedad}_start`], end: memoria[propiedad][`${propiedad}_end`], steps: memoria[propiedad][`${propiedad}_steps`], ease: memoria[propiedad][`${propiedad}_transitionEaseAuxInt`] ? memoria[propiedad][`${propiedad}_transitionEase`] : '' };
